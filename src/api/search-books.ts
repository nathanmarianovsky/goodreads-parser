...
export default async function searchBooks({ q, page, field, }: SearchBooksProps): Promise<SearchBooksResult> {
  ...
  const books = Array.from(trs).map(tr => {
    const el = element(tr),
      ratingStr = el.query('td:nth-child(2) .minirating').text().split('avg rating — ');
    return {
      ...
      publicationYear: el.query('.greyText.smallText.uitext')?.textContent()?.between('published', '—')?.toInt(),
      url: 'https://goodreads.com' + el.query('.bookTitle')?.attr('href')?.split('?')[0],
